...
const AppointmentEmList = () => {
    ...
    async function getAppoinment1() {
        await ApiService.getAppoinmentEm(currentPage, rows)
            .then((response) => {
    ...
    const EmployeeName = (rowData) => {
      ...
      const AppointmentStatus = (rowData) => {
        if (rowData.appointmentStatus === "Employee Canceled") {
            return <div className="badge badge-dark  mr-2">Employee Canceled</div>
        }
    }
...
export default AppointmentEmList;
